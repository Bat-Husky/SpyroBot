...
module.exports = class Coin extends commands {
    ...
    static info (message) {
        const embed = new EmbedBuilder()
              ...
              .addFields([
                { name: "Commandes :", value: "`$coin info` : Affiche cette aide. \n `$coin get` : Permet de gagner 250 coins toutes les heures. \n `$coin amount <user>` : Affiche le nombre de coins d'un utilisateur." },
                { name: '\u200b', value: '\u200b' },
                { name: "Commandes compatibles :", value: "`$Baka` : $baka <user> buy ; outrepasse le cooldown \nCoût : 100 coins \n`$Meme` : $meme buy ; outrepasse le cooldown \nCoût : 100 coins \n`$Skip` : Permet de skip pour les non-Admin \nCoût : 200 coins \n`$AddfaitsDivers` : Permet d'ajouter un Faits Divers pour les non-Admin \nCoût : 1250 coins \n`$TonBotestMalFoutu` : Permet de changer le message pour soit. \nCoût : 750 coins" }
              ])
    ...
    static number (message, coins) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor("#EFEF42")
            .addFields([
                { name: "User", value: cUser.toString() },
                { name: "Number of Coins", value: coins[cUser.id].coins.toString() }
            ])
        message.channel.send({ embeds: [embed] });
    }
}
