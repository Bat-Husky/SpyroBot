...
module.exports = {
    ...
    async execute(interaction, queue, bot) {
        ...
        if (page == totalPage) {
            ...
            const len = (page + 1) * 6 <= inventory[interaction.user.id].cards.length ? 6 : inventory[interaction.user.id].cards.length % 6;

            for (let i = 0; i < len; i++) {
                const element = inventory[user.id].cards[i + (page) * 6];
